...
def computation(source,dest):
	...
	printthis=printthis+("Is Zero Hop?"+str(isZeroHop)+"    ,    ")

	
	if isZeroHop==True:
		printthis=printthis+("Zero Hops using routes with ids:")
		for route in sourceRoutes:
			if route in destRoutes:
				printthis=printthis+(route)+","	
	...
	if isZeroHop==False:
		...
		printthis=printthis+("Is One Hop?"+str(isOneHop)+"    ,    ")
		...
		if isOneHop==True:
			...
			for interchangeStop in interchangableStops:
				...
				for route in destRoutes:
					if interchangeStop in stopsInRoute[route]:
						secondRoute=route;
				printthis=printthis+(str(ctr)+")  firstRoute:"+ firstRoute+ " Interchange at:"+ interchangeStop+" secondRoute:"+secondRoute+"    ,    ")
				ctr=ctr+1
		else:
			printthis=printthis+("No 0 hop or 1 hop routes")
...
def main():
	...
	if request.method == 'POST':
		...
		routes=computation(source, dest)
		return render_template('ans.html',routes=routes)
